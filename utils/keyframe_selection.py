...
def get_pointcloud(depth, intrinsics, w2c, sampled_indices, device):
    CX = intrinsics[0][2]
    CY = intrinsics[1][2]
    FX = intrinsics[0][0]
    FY = intrinsics[1][1]
    ...
    A = torch.abs(torch.round(pts, decimals=4)).to(device)
    B = torch.zeros((1, 3)).to(device).float()
...
def keyframe_selection_overlap(gt_depth, w2c, intrinsics, keyframe_list, k, device="cuda", pixels=1600):
        """
        ...
        pts = get_pointcloud(gt_depth, intrinsics, w2c, sampled_indices, device)
